# this script is used for the "default" simulation in Section 2.2
# we compare the random coefficient model with the aggregated regression approach
...
t.test(x = width.rirs, y = width.agg, paired = TRUE)
# difference is width.rirs - width.agg




results.rirs <- data.frame(left.rirs.ci = ci.left.rirs, right.rirs.ci = ci.right.rirs, left.agg.ci = ci.left.agg,
                         right.agg.ci = ci.right.agg, mean.rirs = est.means.rirs, mean.agg = est.means.agg)

save(results.rirs, file = "data_rirs.rda")
...
pdf("ci_rirs_plot.pdf", width=7, height=7)
...
legend("topright", legend = c("CI RIRS", "CI Agg. OLS", "True slope", "No coverage"), 
