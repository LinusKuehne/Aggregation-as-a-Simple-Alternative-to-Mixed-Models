# model violation 4: not normal but exponential errors
...
results.rirs.vio4 <- data.frame(left.rirs.ci = ci.left.rirs, right.rirs.ci = ci.right.rirs, left.agg.ci = ci.left.agg,
                                right.agg.ci = ci.right.agg, mean.rirs = est.means.rirs, mean.agg = est.means.agg)

save(results.rirs.vio4, file = "data_rirs_vio4.rda")
...
pdf("ci_rirs_plot_vio4.pdf", width=7, height=7)
...
plot(est.means.rirs[1:n.plot], (1:n.plot+0.15), pch = 16, col = "chartreuse3",
     ...
     main = paste(n.plot, "confidence intervals (violation 4)")
)
