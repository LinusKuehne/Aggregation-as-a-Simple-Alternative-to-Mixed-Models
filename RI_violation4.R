...
results.ri.vio4 <- data.frame(left.ri.ci = ci.left.ri, right.ri.ci = ci.right.ri, left.agg.ci = ci.left.agg,
                              right.agg.ci = ci.right.agg, mean.ri = est.means.ri, mean.agg = est.means.agg)

save(results.ri.vio4, file = "data_ri_vio4.rda")
...
pdf("ci_ri_plot_vio4.pdf", width=7, height=7)
