# model violation 1: n.meas is very small
...
results.ri.vio1 <- data.frame(left.ri.ci = ci.left.ri, right.ri.ci = ci.right.ri, left.agg.ci = ci.left.agg,
                         right.agg.ci = ci.right.agg, mean.ri = est.means.ri, mean.agg = est.means.agg)

save(results.ri.vio1, file = "data_ri_vio1.rda")
...
# generate corresp. plot in Fig A.2


n.plot <- min(B, 50)

pdf("ci_ri_plot_vio1.pdf", width=7, height=7)
...
plot(est.means.ri[1:n.plot], (1:n.plot+0.15), pch = 16, col = "chartreuse3",
     ...
     main = paste(n.plot, "confidence intervals (violation 1)")
)
